...
#define kMFStr_RevealInFile(doc, transUnit) ({ \
    auto _name = !doc?nil: [(doc)->ctrl->out_sourceList filenameForTransUnit: (transUnit)]; \
...
#define kMFStr_GoToAll                          stringf(@"Go to '%@'", kMFPath_AllDocuments)
...
#define kMFStr_GoToAll_Symbol           @"document.on.document"
