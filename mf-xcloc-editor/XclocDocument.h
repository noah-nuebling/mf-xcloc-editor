...
//
    ...
    ///
        ...
        XclocDocument *_Nullable getdoc_frontmost(void) { /// Heuristic for accessing the document state from code that isn't specific to a window (mainMenu code) [Oct 2025]
        ...
        NSArray<XclocDocument *> *getdoc_alldocs(void) {
            
            auto result = [NSMutableArray new];
            [NSApp enumerateWindowsWithOptions: NSWindowListOrderedFrontToBack usingBlock:^(NSWindow * _Nonnull w, BOOL * _Nonnull stop) {
                if (isclass(w.windowController, XclocWindowController)) [result addObject: getdoc(w)];
            }];
            return result;
        }
