...
/**
    ...
    Override/define a method on an instance without having to do any subclassing.
        A bit like associatedObjects but for methods.
        [Dec 2025]
...
#define mfimp_begin(ret, args...) \
    ...
    (id) \
    ^id (SEL _cmd, ret (*mfimp_super)(id, SEL _MFIMP_APPEND args)) { \
        return ^ret (id self _MFIMP_APPEND args) {

#define mfimp_end() \
        }; \
    }
#define mfimp_super(args...) \
    mfimp_super(self, _cmd, ##args)
...
BOOL MFSetMethod(id self, SEL sel, id (^mfimp_factory)(SEL _cmd, IMP ogimp)) {
...
@implementation NSObject (MFSetMethod)
    - (BOOL) mf_setMethod: (SEL)sel to: (id (^)(SEL _cmd, IMP ogimp))mfimp_factory {
        return MFSetMethod(self, sel, mfimp_factory);
    }
@end
